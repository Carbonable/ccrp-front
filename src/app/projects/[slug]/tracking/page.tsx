...
export default function TrackingPage({ params }: Readonly<{ params: { slug: string } }>) {
  ...
  useEffect(() => {
    if (dmrvData === undefined || dmrvData === null || dmrvData?.hasOwnProperty('ndvis') === false) return;
      ...
      <div className="relative w-full">
        ...
        <div ref={mapContainer} className="h-[33vh] w-full mt-8 relative">
          ...
          {mapLoaded && <div className="absolute bottom-0 left-0 w-full z-10">
            <TrackingSlider data={dmrvData.ndvis} setSelectedImageIndex={setSelectedImageIndex} selectedDateIndex={selectedDateIndex} setSelectedDateIndex={setSelectedDateIndex} />
          </div>}
        </div>
        {mapLoaded && 
