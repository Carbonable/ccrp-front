...
export default function MapSelect({values, selectedValue, setSelectedValue}: {values: ValueProps[], selectedValue: ValueProps | undefined, setSelectedValue: (v: ValueProps) => void}) {
  ...
  return (
    <Listbox value={selectedValue} onChange={handleSelect}>
      ...
      <Listbox.Button className={({open: boolean}) => `min-w-max flex items-center justify-start w-full cursor-pointer rounded-lg text-neutral-50 py-2 pl-4 pr-10 text-left focus:outline-none border border-neutral-500 bg-opacityDark-70 hover:bg-opacityDark-60`}>
